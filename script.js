...
function showResult(result, completed = true) {
  state.result = result; state.spun = !!completed;
  ...
  try {
    ...
    if (PLAYER.DEMO) {
      ...
      const data = await api("/api/player/spin", {
        ...
        body: { init_data: state.initData, promo_code: $("spinCodeInput").value.trim() },
      });
      winner = data.winner;
      spinsLeft = Number(data.spins_left || 0);
    }
    ...
    await new Promise((resolve) => setTimeout(resolve, 5750)); showResult(winner, spinsLeft <= 0);
    if (spinsLeft > 0) toast(`Spin ${spinsLeft} ကြိမ် ကျန်ပါသေးသည်`, "success");
    if (!winner?.test) {
...
function bind() {
  ...
  $("soundToggle").addEventListener("click", () => {
    ...
    if (state.sound) playMedia("background");
    else Object.values(state.media).forEach((audio) => audio?.pause?.());
...
async function loadBranding() {
  try {
    ...
    const logo = /^https?:\/\//i.test(rawLogo)
      ? rawLogo
      : new URL(`/${rawLogo.replace(/^\/+/, "")}`, location.origin).href;
    document.querySelectorAll(".brand-seal, .announcement-mark").forEach((node) => {
      node.textContent = "";
      node.style.backgroundImage = `url("${logo.replaceAll('"', "%22")}")`;
      node.style.backgroundSize = "contain";
      node.style.backgroundPosition = "center";
      node.style.backgroundRepeat = "no-repeat";
    });
    const audio = branding.audio || {};
    configureMedia("background", audio.background_url, {
      loop: audio.loop !== false,
      volume: Number(audio.volume ?? 0.65),
    });
    configureMedia("spin", audio.spin_url, { volume: Number(audio.volume ?? 0.65) });
    configureMedia("winner", audio.winner_url, { volume: Number(audio.volume ?? 0.65) });
  } catch (_) {}
}

function configureMedia(slot, url, { loop = false, volume = 0.65 } = {}) {
  if (!url) return;
  const media = new Audio(url);
  media.preload = "auto";
  media.loop = !!loop;
  media.volume = Math.max(0, Math.min(Number(volume || 0.65), 1));
  state.media[slot] = media;
}

function playMedia(slot) {
  const media = state.media[slot];
  if (!state.sound || !media) return;
  if (!media.loop) media.currentTime = 0;
  media.play().catch(() => {
    // Mobile browsers require the first play to happen from a user gesture.
  });
}
...
async function boot() {
  ...
  if (promoFromLink) $("spinCodeInput").value = promoFromLink.trim().toUpperCase();
