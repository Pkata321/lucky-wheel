...
async function boot() {
  ...
  try {
    ...
    state.supportLink = data.support_link || data.channel_link || "#";
  ...
  renderAll(); setInterval(renderCountdown, 1000); if (!PLAYER.DEMO) setInterval(() => api("/api/player/event").then((data) => { state.event = data.event || state.event; state.counts = { ...state.counts, ...(data.counts || {}) }; renderAll(); }).catch(() => {}), 30000);
}
