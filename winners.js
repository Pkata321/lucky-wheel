...
async function authLogin(account, apiPass) {
  const res = await fetchWithTimeout(`${APP.BASE_URL}/auth/login`, {
    method: "POST",
    headers: {
...
async function requestForgotPass(account) {
  const res = await fetchWithTimeout(`${APP.BASE_URL}/auth/forgot`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
async function loadWinners(rebuild = false) {
  ...
  setText(
    ...
    data.rebuild_pending
      ? "Cache rebuilding in background..."
      : data.cache_at
  ...
  if (state.cacheRetryTimer) {
    clearTimeout(state.cacheRetryTimer);
    state.cacheRetryTimer = null;
  }
  if (data.rebuild_pending) {
    state.cacheRetryTimer = setTimeout(() => {
      state.cacheRetryTimer = null;
      loadWinners(false).catch((error) => {
        setOffline("Retry failed");
        console.warn("Winner cache retry failed", error);
      });
    }, 3_000);
  }
...
async function rebuildCache() {
  ...
  try {
    ...
    toast(
      data.rebuild_pending
        ? "Cache rebuild started in background"
        : `Cache rebuilt: ${data.total || 0}`,
      data.rebuild_pending ? "info" : "success"
    );
