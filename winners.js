...
const APP = {
  ...
  CACHE_BUSTER: "winner-inbox-premium-v6-1-1",
...
function networkError(error, fallback) {
  if (error instanceof TypeError) {
    return new Error(`${fallback} Backend connection failed. Dashboard files/URL ကို update လုပ်ပါ။`);
  }
  return error;
}
...
async function authLogin(account, apiPass) {
  let res;
  try {
    res = await fetch(`${APP.BASE_URL}/auth/login`, {
      method: "POST",
      credentials: "include",
      cache: "no-store",
      headers: {
...
async function requestForgotPass(account) {
  let res;
  try {
    res = await fetch(`${APP.BASE_URL}/auth/forgot`, {
      method: "POST",
      credentials: "include",
      cache: "no-store",
      headers: {
...
async function handleLogin(event) {
  ...
  if (state.apiKey) localStorage.setItem(APP.STORAGE_KEY, state.apiKey);
  else localStorage.removeItem(APP.STORAGE_KEY);
...
async function logout() {
  ...
  try {
    await fetch(`${APP.BASE_URL}/auth/logout`, {
      method: "POST",
      credentials: "include",
      cache: "no-store",
    });
